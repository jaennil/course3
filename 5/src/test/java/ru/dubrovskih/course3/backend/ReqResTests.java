...
import ru.dubrovskih.course3.backend.data.*;
...
public class ReqResTests extends BaseTest {
    ...
    void createUserTest() {
        ...
        CreateUser actualCreatedUser = checkStatusCodePost("/users", expectedCreatedUser, 201)
                .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("json_schemas/CreateUser.json"))
    ...
    void listResourceTest() {
        ListResourceResponse listResourceResponse = checkStatusCodeGet("/unknown", 200)
                .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("json_schemas/ListResource.json"))
    ...
    void singleResourceTest() {
        SingleResourceResponse singleResourceResponse = checkStatusCodeGet("/unknown/2", 200)
                .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("json_schemas/SingleResource.json"))
    ...
    void registerUnsuccessfulTest() {
        Map response = checkStatusCodePost("/register", Map.of("email", "sydney@fife"), 400)
                .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("json_schemas/Error.json"))
    ...
    void loginSuccessful() {
        ...
        Map loginResponse = checkStatusCodePost("/login", loginData, 200)
                .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("json_schemas/Login.json"))
                .extract().as(Map.class);
    ...
    void delayedResponseTest() {
        ListUsersResponse listUsersResponse = checkStatusCodeGetWithoutTime("/users?delay=3", 200)
                .time(greaterThan(3000L)).and().time(lessThan(5000L))
                .assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath("json_schemas/ListUsers.json"))
                .extract().as(ListUsersResponse.class);
